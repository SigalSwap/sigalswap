...
export function Home() {
  ...
  // Compute TVL from on-chain reserves
  const totalTVL = pools.reduce((sum, p) => {
    if (p.reserve0 == null || p.reserve1 == null) return sum;
    return sum + Number(p.reserve0) / 1e18 + Number(p.reserve1) / 1e18;
  }, 0);

  return (
    <div className="flex flex-col items-center">
      {/* Hero + Swap — fills the viewport below the header */}
      ...
      {/* Stats */}
      <section className="w-full max-w-4xl px-6 py-16" aria-labelledby="stats-heading">
        <h2 id="stats-heading" className="text-2xl font-bold text-center mb-10">
          {t("home.statsTitle")}
        </h2>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {[
            { label: t("home.statsTVL"), value: totalTVL > 0 ? formatUsd(totalTVL) : "—" },
            { label: t("home.statsVolume"), value: "—" },
            { label: t("home.statsPools"), value: pools.length.toString() },
            { label: t("home.statsTrades"), value: "—" },
          ].map(({ label, value }) => (
            <div key={label} className="rounded-xl border border-border bg-card p-5 text-center space-y-1">
